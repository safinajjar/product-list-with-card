  ...
  <teleport to="body">
    <dialog class="modal" open>
      <div class="modal-box">
        ...
        <div class="order-summary" v-if="orderItems.length > 0">
          <div v-for="item in orderItems" :key="item.id" class="order-item">
            <img :src="item.thumbnail" alt="Classic Tiramisu" />
            <div class="item-details">
              <span class="item-name">{{ item.name }}</span>
              <span class="item-quantity text-red">{{ item.quantity }}x</span>
...
.modal {
  ...
  .modal-box {
    @apply col-start-1 row-start-1 flex flex-col rounded-xl bg-white p-10 shadow-2xl sm:w-[90vw] md:w-[70vw] lg:w-[50vw] xl:w-[35vw];
  }
  ...
  .order-summary {
    @apply grid gap-2.5 rounded-xl bg-rose-50 p-5;

    .order-item {
      @apply grid grid-cols-[0.5fr_2fr_1fr];
